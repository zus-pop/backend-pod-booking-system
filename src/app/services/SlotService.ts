  ...
  try {
    ...
    return {
      ...
      message: notAvailableSlots
        .map(
          (notAvailableSlot) =>
            `Slot from ( ${moment(notAvailableSlot.start_time).format(
              "YYYY-MM-DD HH:mm:ss"
            )} ) to ( ${moment(notAvailableSlot.end_time).format(
              "YYYY-MM-DD HH:mm:ss"
  ...
  try {
    ...
    while (startDatetime.isBefore(endDatetime)) {
      ...
      if (overlappingSlots.length) {
        ...
        if (
          moment(slot.start_time).isSameOrBefore(overlappingSlot.start_time) &&
          moment(slot.end_time).isSameOrAfter(overlappingSlot.end_time)
        ) {
          overlapType = `New slot completely encompasses the existing slot`;
        } else if (
          moment(overlappingSlot.start_time).isSameOrBefore(slot.start_time) &&
          moment(overlappingSlot.end_time).isSameOrAfter(slot.end_time)
        ) {
...
const getMonthlyRevenueBySlot = async () => {
  const connection = await pool.getConnection();
  try {
    const monthlyRevenue = await SlotRepo.getMonthlyRevenueBySlot(connection);
...
const getTotalSlotsRefunded = async () => {
  const connection = await pool.getConnection();
  try {
    const totalSlotsRefunded = await SlotRepo.getTotalSlotsRefunded(connection);
