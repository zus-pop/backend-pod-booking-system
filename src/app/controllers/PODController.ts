...
const getAveragePodUsageTime = async (req: Request, res: Response) => {
    try {
        ...
        if (!avgUsageTimes || !avgUsageTimes.length) {
            return res
                .status(404)
                .json({ message: "No POD usage times found" });
        }
...
const getMonthlyRevenueByPOD = async (req: Request, res: Response) => {
    try {
        const monthlyRevenue = await PODService.getMonthlyRevenueByPOD();
        if (!monthlyRevenue || !monthlyRevenue.length) {
            return res.status(404).json({ message: "No revenue data found" });
        }
        return res.status(200).json(monthlyRevenue);
    } catch (error) {
        console.error("Error fetching monthly revenue by POD:", error);
        return res.status(500).json({ message: "Internal server error" });
    }
};

const getTotalPodRevenue = async (req: Request, res: Response) => {
    try {
        const totalPodRevenue = await PODService.getTotalPodRevenue();
        if (!totalPodRevenue) {
            return res.status(404).json({ message: "No revenue data found" });
        }
        return res.status(200).json(totalPodRevenue);
    } catch (error) {
        console.error("Error fetching total pod revenue:", error);
        return res.status(500).json({ message: "Internal server error" });
    }
};
