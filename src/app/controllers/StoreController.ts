...
const getMonthlyRevenueForAllStores = async (req: Request, res: Response) => {
    try {
        const monthlyRevenue =
            await StoreService.getMonthlyRevenueForAllStores();
        if (!monthlyRevenue || !monthlyRevenue.length) {
            return res.status(404).json({ message: "No revenue data found" });
        }
        return res.status(200).json(monthlyRevenue);
...
const getTotalRevenueForAllStores = async (req: Request, res: Response) => {
    try {
        const totalRevenue = await StoreService.getTotalRevenueForAllStores();
        if (!totalRevenue) {
            return res.status(404).json({ message: "No revenue data found" });
        }
        return res.status(200).json(totalRevenue);
    } catch (error) {
        console.error("Error fetching total revenue for all stores:", error);
        return res.status(500).json({ message: "Internal server error" });
    }
};
